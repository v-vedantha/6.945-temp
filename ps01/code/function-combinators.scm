...
(define (simple-spread-combine h f g)   ; p.28
  (let ((n (procedure-arity-min (get-arity f))) (m (procedure-arity-min (get-arity g))))
    (let ((t (+ n m)))
      (define (the-combination . args)
...
(define ((curry-argument i) f)
  (lambda args
    (assert (is-in-range (get-arity f) (+ 1 (length args))))
    (lambda (x)
      (apply f (list-insert args i x)))))
...
(define (permute-arguments . permspec)  ; p.36
  (let ((permute (make-permutation permspec)))
    (lambda (f)
      ...
      (let ((n (get-arity f)))
        (assert (is-in-range n (length permspec)))
...
(define (is-in-range arity nargs)
  (let ((arity-min (procedure-arity-min arity)) (arity-max (procedure-arity-max arity)))
    (if (number? (procedure-arity-max arity))
      (and (>= nargs arity-min) (<= nargs arity-max))
      (>= nargs arity-min)
    )
  )
)

(define (alter-arity arity f)
  (pp f)
  (pp arity)
  (let ((arity-min (procedure-arity-min arity)) (arity-max (procedure-arity-max arity)))
    (let ((new-min (f arity-min)) (new-max (if (number? arity-max) (f arity-max) arity-max)
                                            ))
